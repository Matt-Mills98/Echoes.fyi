...
export default function StickyHeadTable(props) {
    const { aT, setDisplayed, setSelectedPlaylist, setSelectedPlaylistName, setDateAdded, setAlbumMedia, setArtistID, setRows, rows, initPlayingArr, playingArr, setIsPlayingArr, setIndex, setType, setTrackID, trackID } = props;
    ...
    const getFollowedSongs = async (off) => {
        if (!checkAccessToken()) {
            ...
            off = (page - 1) * 50
            setLoading(true);
            await fetch("https://api.spotify.com/v1/me/player/recently-played?limit=50", {
                ...
                .then(async (result) => {
                    if (result.ok) {
                        ...
                        setError(true);
                        setErrorMessage('Failed to retrieve recent tracks');
                        setRowsLocal([]);
                    }
                });
            setLoading(false);
    ...
    const visibleRows = React.useMemo(
        ...
        () => orderBy == 'played_at' ? (stableSortAdded(rowsLocal?.items, getComparator(order, orderBy))
                            ...
                            <Table size="small" stickyHeader aria-label="sticky table" sx={{
                                    ...
                                    <TableRow >
                                        ...
                                        {width > 1400 &&
                                            <TableCell
                                                key={'played_at'}
                                                align={'left'}
                                                sx={{ color: '#FFFFFF', bgcolor: '#16191a', borderBottom: 'none', minWidth: '5%' }} sortDirection={orderBy === 'played_at' ? order : false}
                                            >
                                                <TableSortLabel
                                                    active={orderBy === 'played_at'}
                                                    direction={orderBy === 'played_at' ? order : 'asc'}
                                                    onClick={createSortHandler('played_at')}
                                                    sx={{
                                                        '& .MuiTableSortLabel-icon': {
                                                            color: '#999999 !important',
                                                        },
                                ...
                                <TableBody >
                                    {visibleRows?.map((item, index) => {
                                        ...
                                        return (
                                            <TableRow sx={{
                                                ...
                                                {width > 1400 &&
                                                    <TableCell sx={{ borderBottom: 'none', width: '15%', paddingTop: 0, paddingBottom: 0 }} onClick={(event) => { handleClickOpen(event, item, actIndex, item.track) }}>
                                                        <Typography sx={{ color: '#999999' }} variant="body2">
                                                            {moment(item.played_at).fromNow()}
                                                        </Typography>
                                                    </TableCell>
                                                }
                                                {width > 1000 &&

                                                    <TableCell sx={{ borderBottom: 'none', width: '5%', paddingTop: 0, paddingBottom: 0 }} onClick={(event) => { handleClickOpen(event, item, actIndex, item.track) }}>
                                                        <Typography sx={{ color: '#999999' }} variant="body2">
                                    ...
                                    >
                                        ...
                                        }} component={Link} to={'/Album?id=' + menuItem?.track?.album?.id + '&name=' + menuItem?.track?.album?.name + '&date=' + encodeURIComponent(menuItem?.track?.album?.release_date) + '&media=' + encodeURIComponent(menuItem?.track?.album.images[1].url)}
                                            onClick={() => { window.document.title = 'Profile | ' + menuItem?.track?.album?.name; closeMenu(); updateDisplayed('Albums'); updateSelectedPlaylist(menuItem?.track?.album?.id, menuItem?.track?.album?.name, menuItem?.played_at, menuItem?.track?.album?.images[0]?.url); }}>
                                            <ListItemIcon >
