...
export default function UserPlaylistSongs(props) {
    ...
    const getPlaylistSongs = async (off) => {
        if (!checkAccessToken()) {
            ...
            await fetch("https://api.spotify.com/v1/playlists/" + playlistID + "/tracks?limit=50&offset=" + off, {
                ...
                .then(async (result) => {
                    if (result.ok) {
                        ...
                        setError(true);
                        setErrorMessage('Failed to retrieve playlist tracks');
                        setRowsLocal([]);
                    }
                });
            setLoading(false);
        ...
        window.document.title = from + ' | ' + playlistName;
    ...
    const visibleRows = React.useMemo(
        ...
        ) : (orderBy == 'artist' ? (stableSortAlbum(rowsLocal?.items, getComparator(order, 'name'))
            ...
            {error ? (<Alert severity="error" variant="outlined">
                ...
                <Card sx={{ maxWidth: '100%', bgcolor: '#16191a' }}>
                    <Box sx={{ padding: '15px' }}>
                        ...
                        <TableContainer sx={{
                            maxHeight: '62.5vh', scrollbarWidth: "none" /* Firefox */,
                            ':hover': {
                            ...
                            <Table size="small" stickyHeader aria-label="sticky table" sx={{
                                ...
                                <TableBody >
                                    ...
                                    >
                                        ...
                                        }} component={Link} to={'/Recommendations?id=' + menuItem?.track?.id} >
                                            <ListItemIcon >
                                        ...
                                        }} component={Link} to={'/Album?&type=Albums&id=' + menuItem?.track?.album?.id + '&name=' + menuItem?.track?.album?.name + '&date=' + encodeURIComponent(menuItem?.track?.album?.release_date) + '&media=' + encodeURIComponent(menuItem?.track?.album.images[1].url)}
                                            onClick={() => { closeMenu(); updateDisplayed('Albums'); updateSelectedPlaylist(menuItem?.track?.album?.id, menuItem?.track?.album?.name, menuItem?.added_at, menuItem?.track?.album?.images[0]?.url); }}>
                                            <ListItemIcon >
