...
export default function StickyHeadTable(props) {
    ...
    React.useEffect(() => { checkLikedSongs(); initHoveringArr(rows?.length) }, [rows])

    const checkLikedSongs = async () => {
        if (!checkAccessToken()) {

            let ids = '';
            rows?.items?.map((item) => { ids = ids + item.id + '%2C' });
            if (ids != '') {
    ...
    const visibleRows = React.useMemo(
        ...
        () => orderBy == 'added_at' ? (stableSortAdded(rows?.items, getComparator(order, 'release_date'))
        ) : (orderBy == 'album' ? (stableSortAlbum(rows?.items, getComparator(order, 'name'))
        ) : (orderBy == 'artist' ? (stableSortArtist(rows?.items, getComparator(order, 'name'))
        ) : (stableSort(rows?.items, getComparator(order, orderBy))
        )
        )
        )
        ,
                        ...
                        <TableBody >
                                    ...
                                    <TableRow sx={{
                                                ...
                                                {hovering[index] ? (
                                                    <div>
                                                        {(playingArr[actIndex] && trackID == item.id) ?
                                                            (<IconButton sx={{ color: '#999999' }} onClick={() => { setPlaying(false); setIsPlayingArr(false, rows?.length, actIndex) }}>
                                                                <PauseIcon sx={{ color: '#999999' }} />
                                                            </IconButton>
                                                            ) :
                                                            (<IconButton sx={{ color: '#999999' }} onClick={() => { setPlaying(true); setRows(rows); setTrackID(item.id); setType('search'); setAudIndex(actIndex); setIsPlayingArr(true, rows?.length, actIndex); setIndex(getIndex(item?.id)); }}>
                                        ...
                                        <TableCell sx={{
                                            ...
                                            if (playingArr[actIndex] && trackID == item.id) {
                                                setPlaying(false); setIsPlayingArr(false, rows?.length, actIndex)
                                            }
                                            else {
                                                setPlaying(true); setRows(rows); setTrackID(item.id); setType('search'); setAudIndex(actIndex); setIsPlayingArr(true, rows?.length, actIndex); setIndex(getIndex(item?.id));
                                            }
                                        }}>
                            ...
                            >
                                ...
                                <MenuItem sx={{
                                    ...
                                    onClick={() => { closeMenu(); updateDisplayed('Albums'); updateSelectedPlaylist(menuItem?.album?.id, menuItem?.album?.name, menuItem?.added_at, menuItem?.album?.images[0]?.url); }}>
                                    <ListItemIcon >
